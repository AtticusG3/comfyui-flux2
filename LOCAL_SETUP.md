...
- **`[ERROR] ComfyUI core incomplete: missing comfy/ldm/models/autoencoder.py`**: usually a bad rsync exclude on a persisted ComfyUI tree (unanchored `models/` matched `comfy/ldm/models/`). Rebuild the image (`docker compose up -d --build`), restart, or remove the `comfyui_data` named volume (`docker compose down -v` drops it along with other compose volumes). Current startup uses root-anchored excludes, rsync `protect` filters for bind mounts, re-syncs `comfy/ldm/models` after git apply, and repairs from git before ComfyUI starts.
